...
namespace DAL
{
    public class DataClassHelperBase : IDataClassHelper
    {
            ...
            if (conn is SqlConnection)
            {
                ...
                try
                {
                    using (tran = sqlconn.BeginTransaction())
                    {
                        ...
                        {
                            bulkCopy.DestinationTableName = TableName;  //指定服务器上目标表的名称  
                            ...
                            {
                                bulkCopy.WriteToServer(dt);             //执行把DataTable中的数据写入DB  
                            }
                            tran.Commit();                              //提交事务  
                        }
                    }
        ...
        public void batchUpdata(string tablename, Dictionary<string, object>[] updateList)
        {
            var count = updateList.Length;
            var pagesize = 500;
            int num = count % pagesize == 0 ? count / pagesize : count / pagesize + 1;//页数
            for (int i = 0; i < num; i++)
            {
                var pageList = GetPagedTable(updateList, i + 1, pagesize);
                UpdateObjects(tablename, pageList);//更新数据库
            }
        }
        internal virtual Dictionary<string, object>[] GetPagedTable(Dictionary<string, object>[] updateList, int PageIndex, int PageSize)//PageIndex表示第几页，PageSize表示每页的记录数
        {
            if (PageIndex == 0)
                return updateList;//0页代表每页数据，直接返回

            List<Dictionary<string, object>> newhs = new List<Dictionary<string, object>>();

            int rowbegin = (PageIndex - 1) * PageSize;
            int rowend = PageIndex * PageSize;

            if (rowbegin >= updateList.Length)
                return updateList;//源数据记录数小于等于要显示的记录，直接返回dt

            if (rowend > updateList.Length)
                rowend = updateList.Length;
            for (int i = rowbegin; i <= rowend - 1; i++)
            {
                newhs.Add(updateList[i]);
            }
            return newhs.ToArray();
        }
    }
}
