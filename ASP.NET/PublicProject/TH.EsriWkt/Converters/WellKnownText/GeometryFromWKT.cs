...
namespace TH.EsriWkt.Converters.WellKnownText
{
	/// <summary>
	...
	public class GeometryFromWKT
	{
		...
		public static AgsGeometryBase Parse(string wellKnownText)
		{
        ...
        public static AgsGeometryBase Parse(TextReader reader)
		{
		...
		private static double[][] GetCoordinates(WktStreamTokenizer tokenizer)
		{
            var lsCoordinates = new List<double[]>();
            string nextToken = GetNextEmptyOrOpener(tokenizer);
			if (nextToken == "EMPTY")
				return lsCoordinates.ToArray();

			var externalCoordinate = new double[] { GetNextNumber(tokenizer), GetNextNumber(tokenizer) };
			lsCoordinates.Add(externalCoordinate);
			nextToken = GetNextCloserOrComma(tokenizer);
			while (nextToken == ",")
			{
				var internalCoordinate = new double[] { GetNextNumber(tokenizer), GetNextNumber(tokenizer) };
				lsCoordinates.Add(internalCoordinate);
				nextToken = GetNextCloserOrComma(tokenizer);
			}
			return lsCoordinates.ToArray();
		}
		...
		/// </remarks>
		private static AgsGeometryBase ReadGeometryTaggedText(WktStreamTokenizer tokenizer)
		{
			tokenizer.NextToken();
			string type = tokenizer.GetStringValue().ToUpper();
            AgsGeometryBase geometry;
			switch (type)
			{
				case "POINT":
		...
		private static AgsPolygon ReadMultiPolygonText(WktStreamTokenizer tokenizer)
		{
            AgsPolygon polygons = new AgsPolygon();
			string nextToken = GetNextEmptyOrOpener(tokenizer);
			if (nextToken == "EMPTY")
				return polygons;
            var lsRing = new List<double[][]>();
            AgsPolygon polygon = ReadPolygonText(tokenizer);
            foreach (var ring in polygon.Rings) lsRing.Add(ring);
		...
		///  token is encountered.
		///  </remarks>
		private static AgsPolygon ReadPolygonText(WktStreamTokenizer tokenizer)
		{
		...
		///  token is encountered.
		///  </remarks>
		private static AgsPolygon ReadPolygonText(WktStreamTokenizer tokenizer, bool exteriorRingCCW, bool exteriorRingCCW_Specified)
		{
            AgsPolygon pol = new AgsPolygon();
			string nextToken = GetNextEmptyOrOpener(tokenizer);
			if (nextToken == "EMPTY")
				return pol;
            var polRings = new List<double[][]>();
			...
			{
				polRings.Add(Utilities.Algorithms.IsCCW(exteriorRing) != exteriorRingCCW ? Reverse(exteriorRing) : exteriorRing);
			}
			else
			{
				polRings.Add(exteriorRing);
			}
			...
			while (nextToken == ",")
			{
				...
				polRings.Add(correctedRing); //interior rings
				nextToken = GetNextCloserOrComma(tokenizer);
			}
            pol.Rings = polRings.ToArray();
			return pol;
		}

		private static double[][] Reverse(double[][] pointCollection)
		{
			if (pointCollection == null) throw new ArgumentNullException("pointCollection");
			return pointCollection.Reverse().ToArray();
		}
		...
		/// </remarks>
		private static AgsPoint ReadPointText(WktStreamTokenizer tokenizer)
		{
            AgsPoint p = new AgsPoint();
			string nextToken = GetNextEmptyOrOpener(tokenizer);
			if (nextToken == "EMPTY")
				return p;
		...
		/// ParseException is thrown if an unexpected token is encountered.
		/// </remarks>
		private static AgsMultipoint ReadMultiPointText(WktStreamTokenizer tokenizer)
		{
            AgsMultipoint mp = new AgsMultipoint();
			string nextToken = GetNextEmptyOrOpener(tokenizer);
			if (nextToken == "EMPTY")
				return mp;
            var lsPoints = mp.Points.ToList();
            lsPoints.Add(ReadMapPointText(tokenizer));
			nextToken = GetNextCloserOrComma(tokenizer);
			while (nextToken == ",")
			{
                lsPoints.Add(ReadMapPointText(tokenizer));
        ...
        /// ParseException is thrown if an unexpected token is encountered.
        /// </remarks>
        private static double[] ReadMapPointText(WktStreamTokenizer tokenizer)
        {
		...
		private static AgsPolyline ReadMultiLineStringText(WktStreamTokenizer tokenizer)
		{
            AgsPolyline lines = new AgsPolyline();
			string nextToken = GetNextEmptyOrOpener(tokenizer);
			if (nextToken == "EMPTY")
				return lines;
            var lsPath = new List<double[][]>();
            lsPath.Add(GetCoordinates(tokenizer));
			nextToken = GetNextCloserOrComma(tokenizer);
			while (nextToken == ",")
			{
                lsPath.Add(GetCoordinates(tokenizer));
		...
		/// ParseException is thrown if an unexpected token is encountered.
		/// </remarks>
		private static AgsPolyline ReadLineStringText(WktStreamTokenizer tokenizer)
		{
            AgsPolyline p = new AgsPolyline();
			p.Paths= new double[][][]{ GetCoordinates(tokenizer)};
			return p;
		}
