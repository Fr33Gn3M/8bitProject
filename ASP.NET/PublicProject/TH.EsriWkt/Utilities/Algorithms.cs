...
namespace TH.EsriWkt.Utilities
{
	public class Algorithms
	{
		...
		public static bool IsCCW(double[][] ring)
		{
			double[] PrevPoint, NextPoint;
				double[] p;

			// Check if the ring has enough vertices to be a ring
			if (ring.Length < 3) throw (new ArgumentException("Invalid LinearRing"));
			...
			// Point left to Hip
			int iPrev = hii - 1;
			if (iPrev < 0) iPrev = ring.Length - 2;
			// Point right to Hip
			int iNext = hii + 1;
			if (iNext >= ring.Length) iNext = 1;
