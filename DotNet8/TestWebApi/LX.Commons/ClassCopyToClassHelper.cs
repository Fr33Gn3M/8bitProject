...
namespace LX.Commons
{
    ...
    public static class ClassCopyToClassHelper
    {
        ...
        public static TTarget ModelToTargetModel<TSource, TTarget>(TSource source)
            where TSource : class
            where TTarget : class, new()
        {
            ...
            var sProperties = type1.GetProperties();
            var tProperties = type2.GetProperties();

            foreach (var item in sProperties)
            {
                var tempName = item.Name;
                object value = item.GetValue(source, null);
                if (value != DBNull.Value)
                {
                    foreach (var item2 in tProperties)
                    {
                        if (item2.Name == tempName)
                        {
                            item2.SetValue(obj, value, null);
                            continue;
                        }
                    }
                }
