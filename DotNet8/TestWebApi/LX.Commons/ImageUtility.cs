...
namespace LX.Commons
{
    public class ImageUtility
    {
        ...
        public Bitmap MergeLogoAndString(Bitmap qrImg, Bitmap headerImg, string name, double n = 0.23)
        {
            int margin = 10;
            float dpix = qrImg.HorizontalResolution;
            float dpiy = qrImg.VerticalResolution;
            ...
            g.DrawImage(img1, center.X, center.Y, _headerImg.Width, _headerImg.Height);
            g.Dispose();
            int hp = string.IsNullOrEmpty(name) ? 20 : 120;
            Bitmap backgroudImg = new Bitmap(qrImg.Width + 20, qrImg.Height + hp);
            //backgroudImg.MakeTransparent();
            backgroudImg.SetResolution(dpix, dpiy);
            headerBgImg.SetResolution(dpix, dpiy);
            Graphics g2 = Graphics.FromImage(backgroudImg);
            g2.Clear(Color.White);
            g2.DrawImage(qrImg, 10, 10);
            PointF center2 = new PointF((backgroudImg.Width - headerBgImg.Width) / 2, ((qrImg.Height - headerBgImg.Height) / 2) + 30);
            g2.DrawImage(headerBgImg, center2);

            if (!string.IsNullOrEmpty(name))
            {
                Font font = new Font("GB2312", 22, FontStyle.Regular);//设置字体，大小，粗细
                SolidBrush sbrush = new SolidBrush(Color.Black);//设置颜色
                g2.DrawString(name, font, sbrush, new PointF((Int32)(backgroudImg.Width - name.Length * 40) / 2, qrImg.Height + 30));
            }
            g2.Dispose();
            return backgroudImg;
        }
