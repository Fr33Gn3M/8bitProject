...
namespace Sys.DataBase
{
    ...
    public class ModelTypeConvert<T> where T : class, new()
    {
                    ...
                    if (dt.Columns.Contains(tempName))
                    {
                        ...
                        if (value != null && !string.IsNullOrEmpty(value.ToString()))
                        {
                                ...
                                if (pi.PropertyType == typeof(THGeometry))
                            {
                                ...
                                if (value is string)
                                {
                                    value = new THGeometry() { WKT = value.ToString() };
                                }
                            }
                            if (pi.PropertyType == typeof(DateTime) || pi.PropertyType == typeof(DateTime?))
                                value = DateTime.Parse(value.ToString());
        ...
        public static List<T> ConvertToModelFromType(string nameSpace, string className, DataTable dt, bool isconvertPart = true)
        {
        ...
        public static SqlTableInfo GetSqlTableInfo(T model, Dictionary<string, string> DataBaseKyFieldTableDic, bool isnotconvertPart = false)
        {
            ...
            var dic = ConvertDict2(model, isnotconvertPart);
        ...
        public static Dictionary<string, object> ConvertDict2(T model, bool isconvertPart)
        {
            string tempName = "";
            //获取此模型的公共属性
            PropertyInfo[] propertys = model.GetType().GetProperties();
